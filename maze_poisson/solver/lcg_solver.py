...
class LCGSolver(BaseSolver):
    ...
    def _compute_forces_field(self) -> np.ndarray:
    ...
    def _compute_forces_field_mpi(self) -> np.ndarray:
        """Compute the forces from the field."""
        ...
        self.particles.forces_elec = mpi.all_reduce(forces)
        ...
        #     self.particles.forces_elec[:, axis] = -np.sum(q_neighbors * E_neighbors, axis=1)

        # out = self.particles.forces_elec
        # self.q_tot = c_api.c_compute_force_fd(N, N_p, h, phi, q, neighbors, out)

        return self.particles.forces_elec
