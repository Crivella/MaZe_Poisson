...
def generate_restart(md_variables, grid_setting, output_settings, iter = None):
    ...
    restart_path = os.path.join('restart_files')
    if thermostat == True: 
    ...
    if iter == None:
        filename_output = os.path.join(restart_path, 'restart_N' + str(N) + '_iter' + str(max) + '.csv')
    else:
        filename_output = os.path.join(restart_path, 'restart_N' + str(N) + '_iter' + str(iter) + '.csv')
